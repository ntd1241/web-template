...
const menuGroups = [
  {
    ...
    items: [
      ...
      { icon: PackageCheck, label: 'Tài chính', path: '/' },
      { icon: Boxes, label: 'Tổng quan kho', path: '/' },
    ],
  },
  {
    title: 'Quản trị',
    items: [
      { icon: UserRound, label: 'Hồ sơ cá nhân', path: '/' },
      { icon: Users, label: 'Nhân sự', path: '/' },
      { icon: ClipboardList, label: 'Yêu cầu nhân sự', path: '/' },
      { icon: Building2, label: 'Đối tác', path: '/' },
    ],
  },
  {
    title: 'Vùng sản xuất',
            ...
            <section key={group.title}>
              ...
              <div className="space-y-1">
                {group.items.map((item) => {
                  const active =
                    pathname === item.path && item.label === 'Nhân sự';
