...
export function OrderItemsEditorTable({
  ...
  const bulkFields = [
    ...
    { name: 'discount', label: 'Chiết khấu', kind: 'number' },
    { name: 'note', label: 'Ghi chú', kind: 'text' },
  ] as const;
  const [selectedRowIds, setSelectedRowIds] = useState<string[]>([]);
  const [bulkField, setBulkField] = useState<BulkFieldName>('warehouse');
  const [bulkValue, setBulkValue] = useState('');
  const [headerBulkValues, setHeaderBulkValues] = useState<
    Record<BulkFieldName, string>
  >({
    warehouse: '',
    expiryDate: '',
    taxRate: '',
  ...
  const coerceBulkValue = (fieldName: BulkFieldName, rawValue: string) => {
    switch (fieldName) {
      ...
      case 'expiryDate':
        return rawValue;
      case 'taxRate':
        return Number.isNaN(Number(rawValue)) ? 0 : Number(rawValue);
      case 'discount':
        return Number.isNaN(Number(rawValue)) ? 0 : Number(rawValue);
      case 'note':
        return rawValue;
  ...
  const applyBulkValue = (fieldName: BulkFieldName, rawValue: string) => {
    ...
    selectedIndexes.forEach((index) => {
      form.setValue(`items.${index}.${fieldName}` as never, value as never, {
        shouldDirty: true,
        shouldValidate: true,
      });
    });
      ...
      <div className="flex h-full min-h-0 min-w-0 flex-col">
        ...
        <ScrollArea className="min-h-[360px] min-h-0 flex-1">
          <table className="min-w-[1880px] w-full caption-bottom text-foreground text-sm">
            <TableHeader>
              <TableRow>
                <TableHead className="sticky top-0 z-20 bg-muted w-12">
                  <Checkbox
                ...
                <TableHead className="sticky top-0 z-20 bg-muted w-36">
                  <div className="flex flex-col gap-1.5">
                    <span>Kho</span>
                    <Input
                      aria-label={`Áp dụng kho cho dòng đã chọn`}
                      disabled={selectedIndexes.length === 0}
                      value={headerBulkValues.warehouse}
                      variant="sm"
                      onChange={(event) =>
                        handleHeaderBulkChange('warehouse', event.target.value)
                      }
                    />
                  </div>
                </TableHead>
                ...
                <TableHead className="sticky top-0 z-20 bg-muted w-40">
                  <div className="flex flex-col gap-1.5">
                    <span>Hạn dùng</span>
                    <DatePickerInput
                      aria-label={`Áp dụng hạn dùng cho dòng đã chọn`}
                      calendarLabel={`Chọn hạn dùng cho dòng đã chọn`}
                      disabled={selectedIndexes.length === 0}
                      value={headerBulkValues.expiryDate}
                      valueMode="iso-date"
                      variant="sm"
                      onChange={(value) =>
                        handleHeaderBulkChange('expiryDate', value)
                      }
                    />
                  </div>
                </TableHead>
                ...
                <TableHead className="sticky top-0 z-20 bg-muted w-28 text-right">
                  <div className="flex flex-col gap-1.5">
                    <span>VAT %</span>
                    <Input
                      aria-label={`Áp dụng vat % cho dòng đã chọn`}
                      disabled={selectedIndexes.length === 0}
                      className="text-right tabular-nums"
                      type="number"
                      value={headerBulkValues.taxRate}
                      variant="sm"
                      onChange={(event) =>
                        handleHeaderBulkChange('taxRate', event.target.value)
                      }
                    />
                  </div>
                </TableHead>
                <TableHead className="sticky top-0 z-20 bg-muted w-36 text-right">
                  <div className="flex flex-col gap-1.5">
                    ...
                    <Input
                      aria-label={`Áp dụng chiết khấu cho dòng đã chọn`}
                      disabled={selectedIndexes.length === 0}
                      className="text-right tabular-nums"
                      type="number"
                      value={headerBulkValues.discount}
                      variant="sm"
                      onChange={(event) =>
                        handleHeaderBulkChange('discount', event.target.value)
                      }
                    />
                  </div>
                </TableHead>
                <TableHead className="sticky top-0 z-20 bg-muted min-w-48">
                  <div className="flex flex-col gap-1.5">
                    <span>Ghi chú</span>
                    <Input
                      aria-label={`Áp dụng ghi chú cho dòng đã chọn`}
                      disabled={selectedIndexes.length === 0}
                      value={headerBulkValues.note}
                      variant="sm"
                      onChange={(event) =>
                        handleHeaderBulkChange('note', event.target.value)
                      }
                    />
                  </div>
                </TableHead>
                    ...
                    <TableRow key={field.fieldId}>
                      <TableCell className="px-4 py-2">
                        <Checkbox
                          ...
                          size="sm"
                          onCheckedChange={(checked) =>
                            handleSelectRow(field.fieldId, !!checked)
                          }
                        />
                      </TableCell>
