...
| `dialog`       | Standard dialog shell with title, optional description, body slot, and configured footer actions          | `DialogSpec` (`@/builders/dialog`)            | `npm run gen:dialog -- <spec> <out>`       | building custom dialogs with shared modal chrome        | [`docs/builders/dialog.md`](../../docs/builders/dialog.md)             |
...
| `saved-view` | Typed tenant saved-view config adapter with persisted-filter and column-state normalization | `SavedViewSpec` (`@/builders/saved-view`) | `npm run gen:saved-view -- <spec> <out>` | list pages that share tenant views with filters and column layout | [`docs/builders/saved-view.md`](../../docs/builders/saved-view.md) |
