...
- **Không hardcode màu hex.** Dùng class token `admin-*` (`bg-admin-surface`, `text-admin-blue-dark`,
  `rounded-admin-card`…). Token định nghĩa ở `src/styles/globals.css` (`@theme inline`).
